...
class LiquidityGrabVisualizer:
    ...
    def create_visualization(self) -> go.Figure:
        ...
        # Configure layout
        title = f"<b>Liquidity Grab Analysis</b> | {self.symbol} 1H | {self.data['datetime'].iloc[0].strftime('%Y-%m-%d')} to {self.data['datetime'].iloc[-1].strftime('%Y-%m-%d')}"
    ...
    def save_plot(self, filename: str = None, output_dir: str = None) -> None:
        ...
        if filename is None:
            ...
            filename = f"liquidity_grab_analysis_{self.symbol}_{timestamp}.html"
...
def main():
    ...
    try:
        ...
        visualizer = LiquidityGrabVisualizer('EURUSD')
        ...
        # Save to outputs/plots directory (can be customized)
